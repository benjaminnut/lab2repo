 ...
 * DIntNode.java
 * 
 * A DIntNode provides a node for a doubly linked list with integer data in each
 * node.
 *
 * Lists of nodes can be made of any length, limited only by the amount of free
 * memory in the heap. But beyond Integer.MAX_VALUE
 * 
 * @author Huiping Cao
 *
 *
...
public class DIntNode {
	 ...
	 * @postcondition This node contains the specified data; The prev and next are
	 *                all null references.
	 **/
	 ...
	 * nodes. Note that the initialNext and initialPrev may be null, which indicates
	 * that the new node has nothing before and/or after it.
	 * 
	 * @param initialData: the initial data of this new node
	 ...
	 * 
	 * @param - none
	 * @return a reference to the node before this node (or the null reference if
	 *         there is nothing before this node)
	 **/
	 ...
	 * 
	 * @param newnext: a reference to the node that should appear after this node in
	 *        the linked list (or the null reference if there is no node after this
	 *        node)
	 ...
	 * 
	 * @param newprev a reference to the node that should appear before this node in
	...
	/**
	 * This method increase the value of data by doubling its value This can be used
